...
const editTask = async (editedTask) => {
  try {
    ...
    const res = await put(
      `${import.meta.env.VITE_BASE_URL}/boards/${boardIdRoute}/tasks/${editedTask.id}`,
      selectTask.value,
      token
    )
    //   {
...
<template>
  <div class="flex">
    ...
    <div class="flex flex-col w-screen h-screen items-center bg-gray-200">
                ...
                <table class="w-full whitespace-nowrap">
                  ...
                  <thead class="text-white" style="background-color: #15161a">
                    <tr class="focus:outline-none h-16 text-base">
                      ...
                      <th class="p-3 text-base font-medium tracking-wide text-left">
                        <div class="task-column status flex flex-row items-center">
                          <div>Status</div>
                          <button class="ml-6" @click="sortTasksByCreationTime">
                            <SortDefault />
                          </button>
                          <div @click="toggleSortOrder" class="itbkk-status-sort flex items-center ml-2">
                            <button v-if="sortalp">
                              <SortDown />
                            </button>
                            <button v-if="!sortalp">
                      ...
                      <th class="w-24 p-3 text-base font-medium tracking-wide text-left"></th>
                    </tr>
                  </thead>

                  <!-- body content -->
                  <tbody class="container">
                    <tr v-for="(task, index) in sortedTasks" :key="index" :class="{ 'bg-slate-100': index % 2 === 0 }"
                      class="itbkk-item h-16 box ease-in transition-colors">
                      <td class="itbkk-title p-3 text-base font-medium text-slate-800 truncate">
                        <div class="flex flex-row">
                          <p class="pl-4">{{ index + 1 }}</p>
                          <button @click="openDetails(task.id)"
                      ...
                      <td class="p-3 text-base font-medium text-slate-800 truncate">
                        <div class="itbkk-assignees" :class="{
                      ...
                      <td class="itbkk-status p-3 text-base font-medium text-slate-800 truncate">
                        {{ task.status.name }}
                      </td>
      ...
      <div class="bg-gray-600 h-20">
        <div class="flex justify-end items-center w-11/12 my-6 sm:px-20 absolute bottom-0 right-20">
          <div id="forFilter" v-if="showFilter">
            <div class="flex flex-row">
              <div class="flex">
                <!-- loop -->
                <div class="flex items-center me-4" v-for="(status, id) in statuses" :key="id">
                  <input v-model="statusFilter" id="filter-checkbox" type="checkbox" :value="status.name"
                  ...
                  <label for="filter-checkbox" class="ms-2 text-sm font-medium text-black dark:text-gray-300">{{
                    status.name
                  }}</label>
                </div>
              </div>
  ...
  <div v-if="showDeleteModal">
    <div class="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50">
      <div class="itbkk-message bg-gray-600 border-2 border-slate-200 shadow-lg rounded-2xl p-8 relative w-1/3">
        ...
        <div class="flex justify-end">
          ...
          <button @click="deleteTask(taskToDelete.id), (showDeleteModal = false)"
  ...
  <router-view :task="selectTask" @saveUpdateTask="saveTask" @saveEdit="editTask" @cancelOpe="cancelHandle"
    @failEdit="handelFail" />
</template>
